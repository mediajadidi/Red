...
// این Worker وصل نشده باشد به‌عنوان آخرین راه‌حل امتحان می‌شود — چون آن
// endpoint غیررسمی است و در عمل درخواست‌های ارسالی از IP دیتاسنتر
// Cloudflare را اغلب بی‌صدا رد می‌کند.
async function fetchMT(text, lang, env) {
  if (env.AI) {
    try {
      const r = await env.AI.run('@cf/meta/m2m100-1.2b', { text, source_lang: 'en', target_lang: lang });
...
async function translateOne(text, lang, env) {
  const cacheKey = `tr:v2:${lang}:${normKey(text)}`;
  const cached = await kvGet(env, cacheKey);
  if (cached) return cached;
  const translated = await fetchMT(text, lang, env);
 ...
 * MT معطل نمی‌ماند.
 */
async function translateBatch(uniqueTexts, lang, env, ctx, debugErrors) {
  const map = new Map();
...
async function translatePayload(data, lang, env, ctx, debugErrors) {
  ...
  try {
    ...
    const dynamicMap = needsDynamic.size ? await translateBatch(needsDynamic, lang, env, ctx, debugErrors) : new Map();
